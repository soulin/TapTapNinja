...
public:
	...
	Enemy();
	Enemy(int heroWidthIn, int numberOfTextures, float scalarX, float scalarY, ofImage *blood);
	~Enemy();
...
//this return if and where in range to be killed
    int inRange(int initialSpace, int perfect, int great, int ok, int sloppy, int poor);
...
//whether it shows up on the left or right, left = 2, right = 1
    ...
    float frameSizeX = 100;
    float frameSizeY = 100;
