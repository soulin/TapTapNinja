...
movingRMS::movingRMS() : movingAverage(1,0){
}

movingRMS::movingRMS(int size, float fillValue) : movingAverage(size, fillValue)
{
}

float movingRMS::value() {
	if (cacheInvalid) {
		float total = 0;
		for(int i = 0; i < sampleSize; i++) {
			total += (values[i] * values[i]);
		}
		cachedValue = sqrt(total / (float) sampleSize);
		cacheInvalid = false;
	}
	return cachedValue;
}
