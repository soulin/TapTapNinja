...
void testApp::setup(){
    ...
    scalarX = (float)screenW/(float)menuScreen.width;
    scalarY = (float)screenH/(float)menuScreen.height;
    ...
    printf("Setup: Utility Variables\n");
    ...
    pool->setInfo(scalarX, scalarY);
    
    hero = new Hero();
    //this puts the image in the right place
    hero->setPos(ofGetWidth()/2,ofGetHeight()-(scalarY*310));
    ...
    printf("Loading Enemy Textures\n");
    assign = new enemyTextureAssign(numberOfTextures);
    printf("Loading Background\n");
    background = new Background(numberOfTextures, screenW, screenH);
    printf("Loading BackgroundChangeFlash\n");
    boundaryFlash = new BoundaryChangeFlash(screenW, screenH);
    printf("Loading Comnbo Break\n");
    comboBroken = new ComboBreak(screenW, screenH);
    printf("Load Onscreen Text\n");
    ...
    uberkilled = new UberKilled(scalarX, scalarY);
    songselect = false;
    hasRunOnce = false;
    ...
    if(currentState == playing){
        ...
        if(boundaryChange == true){
            ...
            if(noBoundaryChange == 0){
                ...
                for(int i = 0; i < enemyPointerVector.size(); i++)
                {
                    ...
                    if(enemyPointerVector[i]->returnState() == 6 || enemyPointerVector[i]->returnCheckable()== false)
                    {
                ...
                for(int i = 1 ; i < enemyPointerVector.size(); i ++){
        ...
        if(paused == false){
            ...
            for(int h = 0; h < enemyPointerVector.size(); h++){
                ...
                if(enemyPointerVector[h]->returnCheckable() == true){
                    numberFound = h;
...
void testApp::draw(){
    ...
    switch (currentState){
        ...
        case playing:
            ...
            //the draw order
            hero->drawHero(paused, scalarX, scalarY);
            background->drawForeground();
            background->drawClouds();
            hero->drawSword(scalarX, scalarY);
            ...
            ofDrawBitmapString(ofToString(ofGetFrameRate())+"fps",10,ofGetHeight()-25);
        ...
        if(paused == false){
            if(ableToAttack == true){
                if(touch.x > 512)
                {
                    ...
                    if(uberkilled->returnMeterFull() &&
                       (touch.x > uberkilled->returnPositions(5) &&
                        touch.x < uberkilled->returnPositions(6) &&
                        touch.y > uberkilled->returnPositions(7) &&
                        touch.y < uberkilled->returnPositions(8))){
