...
Enemy :: Enemy(int heroWidthIn, int numberOfTextures, float scalarX, float scalarY, ofImage *blood){
    ...
    flyDeath = false;
    flyDeathCounter = 0;
    yHorizon = ofGetHeight() - (310.0f*sY);
    yPos = yHorizon - frameSizeY;
    bobbing = 0.0f;
    drawing = false;
    twohit = false;
...
void Enemy :: draw(vector<Particle> &particles, int comboSlash, bool paused){
    ...
    if(paused == false){
        if(pauseAnimation == false){
            ...
            if(state == killedAir){
                ...
                yPos = (yHorizon) - (sin(sineMovement)*randomHeight);
    ...
    if(displayIndicator && checkable == true){
        ...
        if(id == 1){
            ofTriangle(xPos + 12,       (yPos-40),
                       xPos + 12 + 30 , (yPos-40),
                       xPos +12 + 15,   (yPos - 40 + 30));
        }
        
        if(id == 2){
            ofTriangle(xPos+8,          (yPos-40),
                       xPos+8 + 30 ,    (yPos -40),
                       xPos +8+15,      (yPos -40+30));
        }
    }
    ...
    if(id == 1){
        enemy->drawSubsection(((float)xPos+(80.0f*sX)), (yPos),
                              -frameSizeX, frameSizeY,
                              animateX, animateY,
                              frameSizeX, frameSizeY);
    }else{
        enemy->drawSubsection(((float)xPos-(30.0f*sX)), (yPos),
                              frameSizeX, frameSizeY,
                              animateX, animateY,
                              frameSizeX, frameSizeY);
    }
...
void Enemy ::drawOnScreen(int givenId){
    ...
    if(id == 1){
        xPos = (1024/2)+(50/2) + ((speed/sX)*180)+ (perfect/2);
        
    }else if(id == 2){
        xPos =  (1024/2)-(50/2)-spriteWidth-((speed/sX)*180) - ((perfect/2));
    }
    xPos*=sX;
