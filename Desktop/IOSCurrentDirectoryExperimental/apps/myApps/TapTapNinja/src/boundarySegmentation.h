...
public:
	double *thisBeatAvg, *beatMean;
	double numAvgs;
	double *distsToMean;
	unsigned int beatFrameCount;
	unsigned int mfccCount;
	deque<int> beatSizes;
	unsigned int boundarySegWindowSize;
	double avgDist, lastAvgDist;
	vectorDelay<doubleVector> mfccBeatAvgs;
	bool canTrigBoundary;
	unsigned int minBeatSize;
	bool overThreshold;
	medianFilter medianAvgDist;
	double threshold;
	
	unsigned int framesAgo; //when the boundary happened (in the past)
	
	boundarySegments(unsigned int mfccCount, unsigned int minBeatSize): mfccCount(mfccCount), minBeatSize(minBeatSize) {init();};
	~boundarySegments();
	void init();
	bool processFrame(bool beat, double *mfccs);
protected:
	boundarySegments(){};
};