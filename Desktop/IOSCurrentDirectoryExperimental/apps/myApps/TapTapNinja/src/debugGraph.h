...
#ifndef _DEBUG_GRAPH_H
#define _DEBUG_GRAPH_H


class debugGraph {
public:
	debugGraph(int windowSize, float min, float max, int topx, int topy, int width, int height, bool lines=false);
	~debugGraph();
	void addVal(float val);
	void addVal(float val, string label);
	void draw();
	void mark();
	...
	void resetMinMax();
	debugGraph(){}
	ofTrueTypeFont font;  //set if using labels
protected:
	int r,g,b,a;
	int x,y,w,h;
	float ymin, ymax;
