...
UberKilled :: UberKilled(){
    ...
    numberToBuildMeter= 40;
    
    //draw the two buttons.
    drawTheTapTaps = false;
    xSizeOfTheTapTaps = 167;
    ySizeOfTheTapTaps = 162;
...
void UberKilled :: draw(bool paused){
    ...
    //draws the bar from the center of the meter ourwards
    tapTapBar.draw((ofGetWidth()/2)-(560/2), yPos);
    tapTapBarBar.drawSubsection((ofGetWidth()/2)-(amountOfMeter/2), yPos+10, amountOfMeter, 80, 0, 0);
