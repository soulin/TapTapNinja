...
inline double euclideanDistance(double *x, double *y, unsigned int count) {
	double tmp[count];
	//t = y - x
...
inline float euclideanDistance(float *x, float *y, unsigned int count) {
	float tmp[count];
	//t = y - x
...
inline double cosineDistance(double *x, double *y, unsigned int count) {
	double dotProd, magX, magY;
	double *tmp = (double*)malloc(count * sizeof(double));
	
	vDSP_dotprD(x, 1, y, 1, &dotProd, count);
	
	vDSP_vsqD(x, 1, tmp, 1, count);
	vDSP_sveD(tmp, 1, &magX, count);
	magX = sqrt(magX);
	
	vDSP_vsqD(y, 1, tmp, 1, count);
	vDSP_sveD(tmp, 1, &magY, count);
	magY = sqrt(magY);
