...
medianFilter::medianFilter(int size, float fillValue):movingAverage(size, fillValue) {
	medianIndex = floor(size / 2.0);
}
...
#ifdef __APPLE_CC__
		vDSP_vsort(&(values[0]), values.size(), 1);
#else
		//sort
		std::sort(tmp.begin(), tmp.end(), SAscFloat());
#endif
		//find median
		cachedValue = tmp[medianIndex];
		cacheInvalid = false;
	}
	return cachedValue;
